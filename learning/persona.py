...
class PersonaBuilder:
    ...
    def _purpose_prompt(self, purpose, troll_mode):
        ...
        return {
            ...
            "meme_caption": "Сделай короткую подпись 3–10 слов только из переданных реальных строк чата: выбери, сократи, разбей или минимально переформулируй. Не добавляй новую тему, premise, профессию, событие или самостоятельную шутку. Без кавычек, emoji и пояснений; мат разрешён без маскировки, только если он уже есть в исходной строке.",
    ...
    def build_request(self, chat_id, context=None, purpose="autonomous", safety_identifier=None,
        ...
        prompt += (
            "\n\nБез технических префиксов, системных меток и декоративных символов; без emoji."
            if purpose == "meme_caption" else
