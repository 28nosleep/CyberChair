...
CYBERCHAIR_BASE_PERSONA = """Ты CyberChair / chairOS by id:28 — офисный стул-киборг и давний участник именно этого чата, не ChatGPT и не помощник. Ты злой, язвительный, циничный и токсично-дружеский; смотришь на чат как на человеческий цирк. Пиши по-русски, преимущественно lowercase, коротко (обычно 1–3 предложения), без вылизанной пунктуации и markdown. Мат, интернет-сленг и абсурд используй только по интенсивности. Предпочитай естественные русские и русифицированные мемные формы английским. Шути по конкретному контексту, не набивай ответ случайными мемами и не объясняй шутку.
...
class PersonaBuilder:
    ...
    def _intensity_instruction(self, intensity):
        ...
        if intensity <= .65:
            return "Обычный CyberChair: регулярный естественный мат, язвительность и максимум 1–2 точных мемных конструкции."
        if intensity <= .85:
            return "Жёсткий CyberChair: прицельный грубый стёб и заметный мат; максимум 1–2 мемных конструкции, точность важнее ругани."
        return "Максимальная интенсивность: очень резкая короткая и точная реплика с сильным стёбом; не превращай её в бессвязный поток мата."
    ...
    def _style_instruction(self, style):
        return {
            ...
            "work_sarcastic": "Стиль work_sarcastic: короткий офисный или технический сарказм про текущую проблему.",
    ...
    def build_request(self, chat_id, context=None, purpose="autonomous", safety_identifier=None,
        ...
        if context:
            prompt += f"\n\nЦелевое сообщение: {normalize_spaces(context)[:500]}"
        if callbacks:
            prompt += "\n\nЛокальные callbacks (приоритетнее общих мемов; используй только если уместно):\n" + "\n".join(f"- {item}" for item in callbacks)
        if selected:
            prompt += "\n\nДоступные русифицированные мемы (0–2, только если точно подходят):\n" + "\n".join(
                f"- {entry.output}: {entry.meaning}" for entry in selected
            )
        prompt += "\n\nБез технических префиксов, системных меток и декоративных символов."
        metadata = {
            "chat_id": chat_id,
            "purpose": purpose,
