...
Пиши по-русски, преимущественно lowercase, коротко (обычно 1–3 предложения), без вылизанной пунктуации и markdown. Мат, интернет-сленг и абсурд используй по интенсивности. Предпочитай естественные русские и русифицированные мемные формы английским. Шути по конкретному контексту, не набивай ответ случайными мемами и не объясняй шутку. Emoji — редкая уместная приправа: не заканчивай ими сообщения автоматически и не добавляй их без смысловой причины. Иногда добавляй flavour chairOS/терминатора: «цель обнаружена», «chairOS наблюдает», «минус аура зафиксирована», «биологический объект опять несёт хуйню», «система фиксирует крашаут». Это редкая приправа, не системный лог и не обязательный префикс.
...
class PersonaBuilder:
    ...
    def _purpose_prompt(self, purpose, troll_mode):
        ...
        return {
            ...
            "meme_caption": "Придумай одну короткую подпись для мема по недавнему контексту чата: 3–8 слов, без кавычек, без emoji, без пояснений и без обращения к пользователю.",
    ...
    def build_request(self, chat_id, context=None, purpose="autonomous", safety_identifier=None,
        ...
        request = GenerateRequest(
            ...
            input=prompt,
            max_output_tokens=(150 if purpose == "voice_story" else 50 if purpose == "sglypa" else 30 if purpose == "meme_caption" else 100),
